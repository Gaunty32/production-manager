...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const { setup2FA, verifyToken, hashBackupCodes } = await import("./twoFactor");

  app.post("/api/auth/2fa/setup", isAuthenticated, requireSuperAdmin, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      ...
      if (user.role !== "super_admin") {
        return res.status(403).json({ error: "2FA is only available for super admins" });
      }

      const { secret, qrCodeUrl, backupCodes } = await setup2FA(user.email || "");
  ...
  app.post("/api/auth/2fa/enable", isAuthenticated, requireSuperAdmin, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      ...
      const user = await storage.enable2FA(userId, secret, hashedBackupCodes);
  ...
  app.post("/api/auth/2fa/disable", isAuthenticated, requireSuperAdmin, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.post("/api/auth/2fa/verify", isAuthenticated, requireSuperAdmin, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { token } = req.body;
  ...
  app.post("/api/auth/2fa/verify-backup", isAuthenticated, requireSuperAdmin, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      ...
      const isValid = await storage.verify2FABackupCode(userId, backupCode);
      res.json({ valid: isValid });
    } catch (error) {
