...
function perfSectionTable(title: string, entityLabel: string, rows: WeeklySummaryTeamRow[] | undefined, weeksAveraged: number): string {
  if (!rows || rows.length === 0) return '';
  return `
    <p style="margin:0 0 8px;font-size:15px;font-weight:800;color:#18181b;">${sanitizeHtml(title)}</p>
    <table style="width:100%;border-collapse:collapse;margin:0 0 20px;border:1px solid #e4e4e7;border-radius:10px;">
      <thead>
        <tr>
          <th style="padding:9px 14px;background:#18181b;font-size:11px;color:#ffffff;text-align:left;text-transform:uppercase;letter-spacing:0.05em;border-top-left-radius:10px;">${sanitizeHtml(entityLabel)}</th>
          ...
          <th style="padding:9px 14px;background:#18181b;font-size:11px;color:#ffffff;text-align:right;text-transform:uppercase;letter-spacing:0.05em;">${weeksAveraged}-wk avg</th>
...
export async function sendWeeklySummaryEmail(params: {
  ...
  const body = `
    ...
    ${perfSectionTable('Team performance — line items completed', 'Team member', params.team, params.weeksAveraged)}
    ${perfSectionTable('Machine performance — line items completed', 'Machine', params.machines, params.weeksAveraged)}
