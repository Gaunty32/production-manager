...
-   **Chat Sample Images**: Staff can attach a sample image directly in any job chat message (image icon button in the input area). Images are uploaded to Object Storage and displayed inline in the message bubble. A "Sample Images" pinned strip appears at the top of the chat panel whenever any message has an image, showing thumbnail previews (clickable for full size). On the customer side, a "Sample Images for Approval" strip appears with the thumbnails and an "Approved, please proceed" quick-reply button that pre-fills the input. The `imageUrl` column on `jobMessages` stores the object key. Both staff and customer message POST endpoints accept an optional `imageUrl` field.
-   **Internal Messages**: Staff can toggle "Team only" mode when replying in job chats. Internal messages (stored with `isInternal: true` on `jobMessages`) are shown with an amber/yellow background and a lock icon. They are completely hidden from customer-facing GET endpoints. The compose bar turns amber when in internal mode with a clear "Customer will not see this message" indicator.
-   **Messages Redesign**: The Staff Messages page (`/messages`) has been redesigned with a tiled customer grid in the Order Chats left panel. Each customer tile shows a coloured avatar (deterministic colour per customer), their name, job count, unread badge, and last message time. Clicking a single-job customer opens the chat directly; multi-job customers drill into a job list. Profile images are shown in message bubbles (staff and customer). Staff profile pictures can be uploaded via the avatar button in the panel header (stored to Object Storage and saved to `users.profileImageUrl`). Customer users also have a `profileImageUrl` column on `customerUsers` with a matching API endpoint. New API endpoints: `GET /api/staff/me`, `GET /api/customer-portal/me`, `PUT /api/staff/me/profile-picture`, `PUT /api/customer-portal/me/profile-picture`.
...
-   **Customer Team Management**: Customers can manage their own portal users from `/customer/team`. They can add new team members (name, email, temporary password), enable/disable individual accounts, and reset passwords. Security: users can only manage members within their own customer account and cannot deactivate their own login. API endpoints: `GET/POST /api/customer-portal/team`, `PATCH /api/customer-portal/team/:id/active`, `POST /api/customer-portal/team/:id/reset-password`.
-   **Customer Invoice History**: Customers can view their billing history at `/customer/invoices`. Shows all invoiced jobs (status `invoiced`) grouped by month, with line item breakdown (job type, description, quantity, stitch count) and dispatch date. API endpoint: `GET /api/customer-portal/invoices`.
